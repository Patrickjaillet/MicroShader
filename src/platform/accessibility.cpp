...
namespace
{
    ...
    {
        if (msg == WM_GETOBJECT && g_root != nullptr && static_cast<long>(lparam) == static_cast<long>(OBJID_CLIENT))
        {
...
void accessibility_init(SDL_Window* window)
{
    ...
    g_hwnd = static_cast<HWND>(SDL_GetPointerProperty(props, SDL_PROP_WINDOW_WIN32_HWND_POINTER, nullptr));
    if (g_hwnd == nullptr)
    {
        return;
    }
