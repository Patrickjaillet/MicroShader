...
void render_diff_view(const std::vector<DiffSpan>& spans)
{
    ...
    for (const DiffSpan& span : spans)
    {
        ...
        ImVec4 color = tokens::text_primary;
        if (span.kind == DiffSpanKind::Removed)
        {
            color = tokens::status_error;
        }
        else if (span.kind == DiffSpanKind::Added)
        {
            color = tokens::status_ok;
        }
        ...
        if (span.kind == DiffSpanKind::Removed)
        {
            ...
            float strike_y = start.y + text_size.y * 0.5f;
            ImDrawList* draw_list = ImGui::GetWindowDrawList();
            draw_list->AddLine(ImVec2(start.x, strike_y), ImVec2(start.x + text_size.x, strike_y), ImGui::GetColorU32(color));
        }
